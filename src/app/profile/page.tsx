...
export default function ProfilePage() {
  ...
  const calculateBMR = () => {
    // Mifflin–St Jeor
    const gender = profile?.personalInfo?.gender; // 'Erkek' | 'Kadın'
    const age = profile?.personalInfo?.age as number | undefined;
    const heightCm = profile?.personalInfo?.height as number | undefined; // cm
    const weightKg = profile?.personalInfo?.weight as number | undefined; // kg
    if (!age || !heightCm || !weightKg) return 0;
    if (gender === 'Erkek') {
      return Math.round(10 * weightKg + 6.25 * heightCm - 5 * age + 5);
    }
    // Varsayılanı kadın formülüne göre uygula
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
            ...
            >
              ...
              <p className="text-2xl font-bold">
                {profile?.personalInfo?.activityLevel ?? 'Orta Aktif'}
              </p>
            </motion.div>
          </div>
          ...
          <div className="bg-white dark:bg-neutral-800 rounded-2xl shadow-xl p-8 mt-8">
            ...
            <div className="border-b border-gray-200 dark:border-gray-700 mb-6 flex gap-4">
              <button
                ...
                className={`pb-4 px-1 border-b-2 font-medium text-sm transition-all duration-300 ${activeTab === 'personal' ? 'border-fitness-blue text-fitness-blue' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'}`}
              >
                Kişisel Bilgiler
              </button>
              <button
                onClick={() => setActiveTab('health')}
                className={`pb-4 px-1 border-b-2 font-medium text-sm transition-all duration-300 ${activeTab === 'health' ? 'border-fitness-green text-fitness-green' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'}`}
              >
                Sağlık Bilgileri
              </button>
              <button
                onClick={() => setActiveTab('settings')}
                className={`pb-4 px-1 border-b-2 font-medium text-sm transition-all duration-300 ${activeTab === 'settings' ? 'border-fitness-orange text-fitness-orange' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'}`}
              >
                Ayarlar
              </button>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      value={profile?.personalInfo?.activityLevel ?? 'moderate'}
                      onChange={(e) =>
            ...
            {activeTab === 'health' && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-2">
                    {(profile?.healthInfo?.conditions ?? []).map(
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-2">
                    {(profile?.healthInfo?.medications ?? []).map(
