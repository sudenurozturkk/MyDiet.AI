...
export default function ChatPage() {
      ...
      messages: [
        {
          id: 1,
          content:
            'Merhaba! MyDiet Ai sağlık asistanınıza hoş geldiniz. Size nasıl yardımcı olabilirim?',
          isUser: false,
          timestamp: new Date(),
        },
      ],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await generateResponse(apiMessages);

      const aiMessage: Message = {
        id: updatedMessages.length + 1,
        content: result?.reply ?? 'Üzgünüm, bir cevap üretilemedi.',
        isUser: false,
        timestamp: new Date(),
      };
      ...
      const errorSessions = sessions.map((s) =>
        s.id === currentSession.id ? { ...s, messages: [...updatedMessages, errorMessage] } : s
      );
  ...
  const loadChatSessions = async (_userEmail: string) => {
    // Harici backend kaldırıldı; local başlangıç oturumu oluştur
    createInitialSession();
  };
      ...
      messages: [
        {
          id: 1,
          content:
            'Merhaba! MyDiet Ai sağlık asistanınıza hoş geldiniz. Size nasıl yardımcı olabilirim? Beslenme, egzersiz, sağlık hedefleri konusunda sorularınızı paylaşabilirsiniz.',
          isUser: false,
          timestamp: new Date(),
        },
      ],
  ...
  const saveChatSessions = async (_sessions: ChatSession[], _activeSessionId?: string) => {
    // Harici backend kaldırıldı; no-op
    return;
  };
                  ...
                  <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl p-4 flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-amber-700 dark:text-amber-300 leading-relaxed">
                        ...
                        Verilen öneriler genel bilgi amaçlıdır ve hata yapabilir. Sağlık
                        sorunlarınız için mutlaka <strong>doktorunuza danışınız</strong>. Acil
                    ...
                    {currentSession.messages.map((message) => (
                      ...
                      >
                        ...
                        >
                          ...
                          {/* Mesaj Balonu */}
                          <div
                            className={`relative px-5 py-4 rounded-3xl shadow-lg transition-all duration-200 hover:shadow-xl ${
                              message.isUser
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex-1 min-w-0">
                            ...
                            >
                              ...
                              {session.createdAt.toLocaleDateString('tr-TR')}
                            </p>
                          </div>
